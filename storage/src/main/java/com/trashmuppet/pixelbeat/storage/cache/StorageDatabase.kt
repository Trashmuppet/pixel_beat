 ...
 * filesystem walk.
 *
 * `exportSchema = true` + the `room.schemaLocation` KSP arg in
 * `storage/build.gradle.kts` emit `1.json` / `2.json` so the
 * migration test in [StorageDatabaseMigrationTest] can validate
 * v1 → v2 with `MigrationTestHelper`. Migration is still
 * **additive + safe** even on devices where `MIGRATION_1_2` fails
 * to run — see `fallbackToDestructiveMigration()` guardrail.
 *
 * Room 2.7.2 + KSP2 supports anonymous `object : Migration(...)`
 * declarations inside companion objects; the top-level class
 * extraction from the prior investigation round is reverted.
 */
@Database(
