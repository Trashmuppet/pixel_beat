...
import com.trashmuppet.pixelbeat.scene.void.VoidScene
...
class SceneRuntimeTest {
    ...
    @Test
    fun `VoidScene is byte-deterministic across runs`() {
        val project = fixture()
        val stateA = runVoidOnce(project)
        val stateB = runVoidOnce(project)
        assertArrayEquals(
            stateA.pixels, stateB.pixels,
            "Two equal VoidScene runs must produce identical pixel arrays"
        )
        assertEquals(stateA.tick, stateB.tick, "tick counters must match")
        // VoidScene has empty background so we only assert hit shapes rendered.
        assertTrue(
            stateA.pixels.any { byte -> (0..7).any { ((byte.toInt() ushr (7 - it)) and 1) == 1 } },
            "VoidScene should render at least one hit shape pixel"
        )
    }

    private fun runVoidOnce(project: MBeatProject): SceneRenderState =
        VoidScene().apply { load(project) }.let { scene ->
            val sim = AnimationSystem(scene)
            sim.enqueueHit(HitEvent(tick = 200, trackId = "blank/kick"))
            sim.enqueueHit(HitEvent(tick = 400, trackId = "blank/snare"))
            sim.enqueueHit(HitEvent(tick = 600, trackId = "blank/hat"))
